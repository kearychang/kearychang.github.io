...
def resize_and_convert_image(file_path):
	try:
		...
		#convert if png
		isPng = file_path.lower().endswith('.png')
		isWebp = file_path.lower().endswith('.webp')
		if isPng or isWebp:
			jpg_path = os.path.splitext(file_path)[0] + ".jpg"
			img = Image.open(file_path)
			if img.mode != "RGB":  #OSError: cannot write mode P as JPEG
				img = img.convert("RGB")
			img.save(jpg_path, "JPEG", quality=(70 if isPng else 100))
...
def process_folder(folder_path):
	with concurrent.futures.ThreadPoolExecutor() as executor:
		for root, _, files in os.walk(folder_path):
			image_files = [file_name for file_name in files if file_name.lower().endswith(('.png', '.jpg', '.jpeg', 'webp'))]
